export const enUSMessages = {
  'app.name': 'Illyawish',
  'common.language': 'Language',
  'common.loading': 'Loading...',
  'common.cancel': 'Cancel',
  'common.close': 'Close',
  'common.error': 'Error',
  'common.reset': 'Reset',
  'common.edit': 'Edit',
  'common.delete': 'Delete',
  'common.saving': 'Saving...',
  'common.loadMore': 'Load more',

  'app.loading': 'Loading...',
  'app.loadingWorkspace': 'Loading your workspace...',

  'login.workspaceBadge': 'Illyawish',
  'login.heroTitle':
    'Illyawish opens a small moonlit workshop where the Nasuverse and Prisma Illya leave their spell circles glowing in your browser.',
  'login.heroDescription':
    'Sign in like you are sealing a mystic contract. Behind the magic crest, a Go backend, SQLite memory, and streaming replies keep each conversation ready for its next incantation.',
  'login.featureStreamingTitle': 'Streaming',
  'login.featureStreamingDescription':
    'Assistant tokens arrive incrementally, so the UI feels alive instead of blocked.',
  'login.featurePersistentTitle': 'Persistent',
  'login.featurePersistentDescription':
    'Conversation history stays in SQLite and comes back after refresh.',
  'login.featureFocusedTitle': 'Focused',
  'login.featureFocusedDescription':
    'A quiet Illyawish workspace built for reading and long-form prompts.',
  'login.signIn': 'Sign in',
  'login.continueAs': 'Continue as {{name}}',
  'login.prefilledDescription':
    'Sign in with your workspace account to continue.',
  'login.yourAccount': 'your account',
  'login.bootstrapEyebrow': 'Initialize workspace',
  'login.bootstrapTitle': 'Create the first administrator',
  'login.bootstrapDescription':
    'This workspace has not been initialized yet. Create the first admin account to finish setup.',
  'login.creatingAccount': 'Creating account...',
  'login.createFirstAdmin': 'Create first admin',
  'login.username': 'Username',
  'login.password': 'Password',
  'login.signingIn': 'Signing in...',
  'login.enterWorkspace': 'Enter workspace',
  'login.error': 'Unable to sign in right now',

  'chat.expandSidebar': 'Expand conversation sidebar',
  'chat.collapseSidebar': 'Collapse conversation sidebar',
  'chat.newConversation': 'New conversation',
  'chat.defaultModel': 'Default model',
  'chat.export': 'Export',
  'chat.settings': 'Settings',
  'chat.admin': 'Admin',
  'chat.regenerate': 'Regenerate',
  'chat.stop': 'Stop',
  'chat.openSidebar': 'Open conversation sidebar',
  'chat.closeSidebar': 'Close conversation sidebar',
  'chat.sidebarNavigation': 'Conversation sidebar',
  'chat.loadingConversation': 'Loading conversation...',
  'chat.editingBanner':
    'Editing your latest message. Sending will regenerate the reply.',
  'chat.updateMessagePlaceholder': 'Update your message...',
  'chat.messagePlaceholder': 'Message Illyawish...',
  'chatContext.providerModelLabel': 'Provider and model',
  'chatContext.providerLabel': 'Provider',
  'chatContext.noProviderOptions': 'No provider presets available',
  'chatContext.selectProviderFirst': 'Select a provider first',
  'chatContext.knowledgeDisabled': 'Knowledge disabled',
  'chatContext.knowledgeEnabled': 'Knowledge enabled · {{count}} spaces',
  'chatContext.workflowDisabled': 'Workflow disabled',
  'chatContext.workflowEnabled': 'Workflow enabled · {{name}}',
  'chatContext.setAsDefault': 'Set as default',
  'chat.attachFile': 'Attach file',
  'chat.removeAttachment': 'Remove attachment {{name}}',
  'chat.sendMessage': 'Send message',
  'chat.toolsTrigger': 'Tools',
  'chat.expandComposer': 'Expand editor',
  'chat.collapseComposer': 'Collapse editor',
  'chat.shortcutHint': 'Enter to send, Ctrl+Enter for a new line',
  'chat.exportDefaultTitle': 'conversation',

  'sidebar.newChat': 'New chat',
  'sidebar.searchPlaceholder': 'Search conversations',
  'sidebar.active': 'Active',
  'sidebar.archived': 'Archived',
  'sidebar.recents': 'Recents',
  'sidebar.noConversations': 'No conversations yet.',
  'sidebar.pinnedPrefix': 'Pinned · ',
  'sidebar.pin': 'Pin',
  'sidebar.unpin': 'Unpin',
  'sidebar.rename': 'Rename',
  'sidebar.restore': 'Restore',
  'sidebar.archive': 'Archive',
  'sidebar.deleteConversation': 'Delete conversation {{title}}',
  'sidebar.moreActions': 'More actions for {{title}}',
  'sidebar.hideActions': 'Hide actions for {{title}}',
  'sidebar.signOut': 'Sign out',
  'sidebar.foldersTitle': 'Folders',
  'sidebar.allFolders': 'All folders',
  'sidebar.unfiled': 'Unfiled',
  'sidebar.tagsTitle': 'Tags',
  'sidebar.selectionMode': 'Select',
  'sidebar.selectionDone': 'Done',
  'sidebar.selectedCount': '{{count}} selected',
  'sidebar.moveToFolder': 'Move to folder',
  'sidebar.addTags': 'Add tags',
  'sidebar.removeTags': 'Remove tags',
  'sidebar.moveSelected': 'Move selected',
  'sidebar.addTagsSelected': 'Add tags to selected',
  'sidebar.removeTagsSelected': 'Remove tags from selected',
  'sidebar.bulkUpdateSuccess': 'Updated {{count}} conversations',
  'sidebar.bulkUpdatePartial': 'Updated {{successCount}} conversations, {{failureCount}} failed',

  'settings.title': 'Settings',
  'settings.chatDescription':
    'Set a global prompt for your account and an optional session prompt for the current conversation or the next new chat.',
  'settings.historyDescription':
    'Control which conversations appear in history and manage bulk history actions without crowding the sidebar.',
  'settings.providerDescription':
    'Manage your saved AI provider presets. The active preset is used for new requests, while the server fallback from data/app.json remains available.',
  'settings.chatTab': 'Chat',
  'settings.historyTab': 'History',
  'settings.providerTab': 'AI Provider',
  'settings.ragTab': 'RAG',
  'settings.knowledgeTab': 'Knowledge',
  'settings.workflowTab': 'Workflow',
  'settings.securityTab': 'Security',
  'settings.languageTab': 'Language',
  'settings.transferTab': 'Import / Export',
  'settings.ragDescription':
    'Configure the embedding and reranking provider used by local RAG.',
  'settings.knowledgeDescription':
    'Manage user-private knowledge spaces and ingested documents.',
  'settings.workflowDescription':
    'Choose built-in workflow templates and save private presets.',
  'settings.languageDescription':
    'Choose the interface language used across the workspace. Changes apply immediately.',
  'settings.securityDescription':
    'Change your password and revoke active sessions for this account.',
  'settings.securityChangePasswordTitle': 'Change password',
  'settings.securityChangePasswordDescription':
    'Update your current password and keep your workspace account secure.',
  'settings.securityCurrentPassword': 'Current password',
  'settings.securityNewPassword': 'New password',
  'settings.securitySubmitPasswordChange': 'Change password',
  'settings.securityPasswordChanged': 'Password updated.',
  'settings.securitySessionTitle': 'Session security',
  'settings.securitySessionDescription':
    'Sign out every active session for this account. You will need to log in again on all devices.',
  'settings.securityLogoutAllSessions': 'Log out all sessions',
  'settings.transferDescription':
    'Export the current conversation as Markdown, or import a Markdown file as a new conversation.',
  'settings.historyViewSection': 'History view',
  'settings.historyViewHelp':
    'Switch between active and archived conversations for the sidebar list.',
  'settings.historyFiltersSection': 'History filters',
  'settings.historyFiltersHelp':
    'Filter the loaded conversation history by folder or tag.',
  'settings.historyNoTags': 'No tags are available for the current history view.',
  'settings.bulkManageSection': 'Bulk management',
  'settings.bulkManageHelp':
    'Enter selection mode here, then choose conversations from the sidebar list for bulk actions.',
  'settings.languageHelp':
    'Switch the interface language here. Your selection is saved locally for the next visit.',
  'settings.exportSectionTitle': 'Export conversation',
  'settings.exportReadyDescription':
    'Export the current conversation "{{title}}" as a Markdown file.',
  'settings.exportEmptyDescription':
    'Open a conversation with messages before exporting it as Markdown.',
  'settings.importSectionTitle': 'Import conversation',
  'settings.importDescription':
    'Import a Markdown file previously exported from this workspace as a new conversation.',
  'settings.importConversation': 'Import Markdown',
  'settings.importingConversation': 'Importing...',
  'settings.importSuccess': 'Conversation imported',
  'settings.model': 'Model',
  'settings.modelPlaceholder': 'Use provider default model if left blank',
  'settings.modelDefaultOption': 'Use provider default model',
  'settings.globalModelSection': 'Global model settings',
  'settings.globalModelSectionHelp':
    'These defaults apply across conversations until you change them again.',
  'settings.globalPrompt': 'Global prompt',
  'settings.globalPromptPlaceholder': 'Leave blank to disable the global prompt',
  'settings.promptSection': 'Prompt settings',
  'settings.sessionPrompt': 'Session prompt',
  'settings.sessionPromptHelp':
    'Leave blank to fall back to the global prompt. A non-empty session prompt overrides the global prompt for this conversation.',
  'settings.sessionPromptPlaceholder':
    'Leave blank to use the global prompt for this conversation',
  'settings.folder': 'Folder',
  'settings.folderPlaceholder': 'Optional workspace, project, or topic',
  'settings.tags': 'Tags',
  'settings.tagsPlaceholder': 'Comma-separated tags',
  'settings.tagsHelp':
    'Use commas to organize conversations by workstream, owner, or urgency.',
  'settings.systemPrompt': 'System prompt',
  'settings.temperature': 'Temperature',
  'settings.maxTokens': 'Max output tokens',
  'settings.contextWindowTurns': 'Context window (recent turns)',
  'settings.contextWindowTurnsHelp':
    'Leave blank to keep the full conversation history. This version trims by recent turns, while the model itself may still enforce a hard limit.',
  'settings.currentSource': 'Current source',
  'settings.savedPresets': 'Saved presets',
  'settings.onePresetActive': 'One preset can be active at a time.',
  'settings.createProviderPreset': 'Create a new provider preset',
  'settings.loadingPresets': 'Loading provider presets...',
  'settings.active': 'Active',
  'settings.key': 'Key',
  'settings.setActive': 'Set active',
  'settings.noSavedPresets':
    'No saved provider presets yet. Create one to use a custom AI provider endpoint.',
  'settings.serverFallback': 'Server fallback',
  'settings.serverReady': 'Ready to use when no preset is active.',
  'settings.serverNotConfigured': 'Not configured in data/app.json.',
  'settings.noFallbackBaseUrl': 'No fallback base URL',
  'settings.noFallbackModel': 'No fallback model',
  'settings.editPreset': 'Edit preset',
  'settings.newPreset': 'New preset',
  'settings.presetDescription':
    'Save a format, model, base URL, and API key for any supported AI provider.',
  'settings.providerFormat': 'Provider format',
  'settings.providerFormatOpenAI': 'openai',
  'settings.providerFormatAnthropic': 'anthropic',
  'settings.providerFormatGemini': 'gemini',
  'settings.presetName': 'Preset name',
  'settings.presetNamePlaceholder': 'My AI provider',
  'settings.modelList': 'Available models',
  'settings.modelListDescription':
    'Add one or more models for this provider, then choose which one is the default.',
  'settings.addModel': 'Add model',
  'settings.removeModel': 'Remove model',
  'settings.defaultModelLabel': 'Default model',
  'settings.defaultModelEmpty': 'Add a model first',
  'settings.baseUrl': 'Base URL',
  'settings.apiKey': 'API key',
  'settings.apiKeyPlaceholderEdit': 'Leave blank to keep the current key',
  'settings.apiKeyPlaceholderEditHint': 'Leave blank to keep {{hint}}',
  'settings.apiKeyPlaceholderNew': 'sk-...',
  'settings.apiKeyPlaceholderNewReuse':
    'Leave blank to reuse the active preset key',
  'settings.apiKeyHelpEdit':
    'Leave this blank to keep the existing encrypted API key, or enter a new one to replace it.',
  'settings.apiKeyHelpNew':
    'Stored encrypted by the backend after saving.',
  'settings.apiKeyHelpNewReuse':
    'Leave this blank to reuse the current active preset API key, or enter a new one to store a separate key.',
  'settings.apiKeyStoredHint': 'Stored key: {{hint}}',
  'settings.noStoredKey': 'not stored',
  'settings.saveSettings': 'Save settings',
  'settings.resetForm': 'Reset form',
  'settings.savePreset': 'Save preset',
  'settings.createPreset': 'Create preset',
  'settings.testConnection': 'Test connection',
  'settings.testingConnection': 'Testing...',
  'settings.validationPresetNameRequired': 'Enter a preset name',
  'settings.validationBaseUrlRequired': 'Enter a base URL',
  'settings.validationApiKeyRequired': 'Enter an API key',
  'settings.validationProviderFormatRequired': 'Choose a provider format',
  'settings.validationModelRequired': 'Enter a model name',
  'settings.validationModelListRequired': 'Add at least one model',
  'settings.validationDefaultModelRequired': 'Choose a default model',
  'settings.validationDefaultModelInvalid':
    'The default model must be one of the available models',

  'rag.activeProviderTitle': 'Active RAG Provider',
  'rag.activeProviderDescription':
    'Embedding and reranking use a dedicated provider separate from chat models.',
  'rag.noKeyHint': 'no key hint',
  'rag.active': 'Active',
  'rag.activate': 'Activate',
  'rag.noSavedPresets': 'No saved RAG provider presets yet.',
  'rag.createProviderTitle': 'Create RAG Provider',
  'rag.providerName': 'Provider name',
  'rag.providerNamePlaceholder': 'SiliconFlow',
  'rag.baseUrl': 'Base URL',
  'rag.apiKey': 'API key',
  'rag.embeddingModel': 'Embedding model',
  'rag.rerankerModel': 'Reranker model',
  'rag.saveProvider': 'Save provider',

  'knowledge.spacesTitle': 'Knowledge Spaces',
  'knowledge.spacesDescription':
    'Choose which spaces should be queried for this conversation.',
  'knowledge.noSpaces': 'No knowledge spaces yet.',
  'knowledge.noDescription': 'No description',
  'knowledge.spaceEnabled': 'Enabled',
  'knowledge.spaceDisabled': 'Disabled',
  'knowledge.spaceSaving': 'Saving...',
  'knowledge.enableSpaceAction': 'Enable knowledge space {{name}}',
  'knowledge.disableSpaceAction': 'Disable knowledge space {{name}}',
  'knowledge.createSpaceTitle': 'Create Knowledge Space',
  'knowledge.createSpaceDescription':
    'Create a private space to organize imported notes, webpages, and files.',
  'knowledge.spaceName': 'Space name',
  'knowledge.spaceNamePlaceholder': 'Engineering notes',
  'knowledge.spaceDescription': 'Space description',
  'knowledge.spaceDescriptionPlaceholder': 'Optional summary for this space',
  'knowledge.editSpaceTitle': 'Edit Knowledge Space',
  'knowledge.editSpaceDescription': 'Update the selected knowledge space name and summary.',
  'knowledge.editSpace': 'Edit space',
  'knowledge.deleteSpace': 'Delete space',
  'knowledge.saveSpace': 'Save space',
  'knowledge.cancelSpaceEdit': 'Cancel space edit',
  'knowledge.createSpace': 'Create space',
  'knowledge.addDocumentTitle': 'Add Knowledge Document',
  'knowledge.addDocumentDescription':
    'Import text, webpages, or attachments into the selected knowledge space.',
  'knowledge.editDocumentTitle': 'Edit Knowledge Document',
  'knowledge.editDocumentDescription':
    'Update document metadata or replace attachment files for the selected space.',
  'knowledge.documentSpace': 'Knowledge space',
  'knowledge.chooseSpace': 'Choose a space',
  'knowledge.sourceText': 'Text',
  'knowledge.sourceURL': 'URL',
  'knowledge.sourceAttachment': 'Attachment',
  'knowledge.documentTitle': 'Document title',
  'knowledge.documentTitlePlaceholder': 'Release checklist',
  'knowledge.documentContent': 'Document content',
  'knowledge.documentContentOptional': 'Document content (optional)',
  'knowledge.textContentPlaceholder':
    'Paste plain text or markdown here',
  'knowledge.sourceURLField': 'Source URL',
  'knowledge.sourceURLPlaceholder': 'https://example.com/article',
  'knowledge.urlContentPlaceholder':
    'Optional: paste webpage text here, or leave blank to fetch from the URL',
  'knowledge.uploadFiles': 'Upload knowledge files',
  'knowledge.uploadReplacementFile': 'Upload replacement file',
  'knowledge.replaceFileHint': 'Leave empty to keep the current attachment file.',
  'knowledge.uploadHint': 'Supports PDF, Markdown, and TXT. Multiple files are allowed.',
  'knowledge.selectedFiles': 'Selected files',
  'knowledge.addDocument': 'Add document',
  'knowledge.editDocument': 'Edit document',
  'knowledge.deleteDocument': 'Delete document',
  'knowledge.replaceFile': 'Replace file',
  'knowledge.saveDocument': 'Save document',
  'knowledge.cancelDocumentEdit': 'Cancel document edit',
  'knowledge.documentsTitle': 'Documents',
  'knowledge.refreshDocuments': 'Refresh',
  'knowledge.noDocuments': 'No documents loaded for this space.',
  'knowledge.chunkSuffix': 'chunks',

  'workflow.presetsTitle': 'Workflow Presets',
  'workflow.presetsDescription':
    'Select a workflow preset to turn this conversation into an agent run.',
  'workflow.noPresetOption': 'No workflow preset',
  'workflow.createPresetTitle': 'Create Workflow Preset',
  'workflow.presetName': 'Preset name',
  'workflow.presetNamePlaceholder': 'Knowledge Q&A',
  'workflow.templateLabel': 'Workflow preset',
  'workflow.templateOptionLabel': 'Workflow template',
  'workflow.createPreset': 'Create preset',
  'workflow.savePreset': 'Save preset',
  'workflow.editPresetTitle': 'Edit Workflow Preset',
  'workflow.formDescription': 'Save a reusable workflow preset for this conversation.',
  'workflow.savedPresetsTitle': 'Saved presets',
  'workflow.savedPresetsDescription': 'Edit or remove workflow presets saved to your workspace.',
  'workflow.noSavedPresets': 'No workflow presets saved yet.',
  'workflow.newPreset': 'New preset',
  'workflow.selectedPresetBadge': 'Selected',
  'workflow.selectedPresetSummary': 'Selected preset: {{name}} · {{template}}',
  'workflow.editPresetAction': 'Edit preset {{name}}',
  'workflow.deletePresetAction': 'Delete preset {{name}}',
  'workflow.validationPresetNameRequired': 'Enter a preset name',

  'executionPanel.title': 'Execution Panel',
  'executionPanel.description':
    'Live workflow, retrieval, and tool activity for the current run.',
  'executionPanel.approve': 'Approve',
  'executionPanel.reject': 'Reject',
  'executionPanel.showTimeline': 'Show timeline',
  'executionPanel.hideTimeline': 'Hide timeline',
  'executionPanel.showDetails': 'Show details',
  'executionPanel.hideDetails': 'Hide details',
  'executionPanel.none': 'None',
  'executionPanel.executionProgress': 'Execution progress',
  'executionPanel.currentStage': 'Current stage',
  'executionPanel.summaryTitle': 'Runtime summary',
  'executionPanel.stageRailTitle': 'Runtime stages',
  'executionPanel.stepsTitle': 'Workflow steps',
  'executionPanel.retrievalTitle': 'Knowledge retrieval',
  'executionPanel.toolsTitle': 'Tool calls',
  'executionPanel.timelineTitle': 'Timeline',
  'executionPanel.timelineDescription': 'Detailed runtime events for this run.',
  'executionPanel.confirmationNeeded': 'This action is waiting for confirmation.',
  'executionPanel.collapsedSummary': '{{completed}} / {{total}} steps complete',
  'executionPanel.summary.status': 'Status',
  'executionPanel.summary.currentStep': 'Current step',
  'executionPanel.summary.progress': 'Progress',
  'executionPanel.summary.template': 'Workflow',
  'executionPanel.progressValue': '{{completed}}/{{total}} steps',
  'executionPanel.retrievalCompact': '{{count}} retrievals',
  'executionPanel.toolCompact': '{{count}} tools',
  'executionPanel.retrievalStats': '{{results}} results · {{citations}} citations · {{spaces}} spaces',
  'executionPanel.retrievalMetric.results': 'Results',
  'executionPanel.retrievalMetric.citations': 'Citations',
  'executionPanel.retrievalMetric.spaces': 'Spaces',
  'executionPanel.citationsTitle': 'References',
  'executionPanel.citationOverflow': '+{{count}}',
  'executionPanel.status.running': 'Running',
  'executionPanel.status.completed': 'Completed',
  'executionPanel.status.waiting_confirmation': 'Waiting for confirmation',
  'executionPanel.status.failed': 'Failed',
  'executionPanel.status.cancelled': 'Cancelled',
  'executionPanel.tool.http_request': 'HTTP request',
  'executionPanel.tool.fetch_url': 'Fetch URL',
  'executionPanel.tool.knowledge_search': 'Knowledge search',
  'executionPanel.tool.text_transform': 'Text transform',
  'executionPanel.step.question': 'Question understanding',
  'executionPanel.step.retrieve_knowledge': 'Knowledge retrieval',
  'executionPanel.step.compose_answer': 'Compose answer',
  'executionPanel.step.finalize': 'Finalize response',
  'executionPanel.template.knowledge_qa': 'Knowledge Q&A',
  'executionPanel.template.webpage_digest': 'Webpage Digest',
  'executionPanel.template.document_summary': 'Document Summary',
  'executionPanel.template.multi_document_compare': 'Multi-document Comparison',
  'executionPanel.template.structured_extraction': 'Structured Extraction',
  'executionPanel.stepLabel': 'Step: {{name}}',
  'executionPanel.toolLabel': 'Tool: {{name}}',
  'executionPanel.citationsLabel': 'Citations: {{names}}',
  'executionPanel.event.run_started': 'Run started',
  'executionPanel.event.workflow_step_started': 'Workflow step started',
  'executionPanel.event.workflow_step_completed': 'Workflow step completed',
  'executionPanel.event.retrieval_started': 'Retrieval started',
  'executionPanel.event.retrieval_completed': 'Retrieval completed',
  'executionPanel.event.tool_call_started': 'Tool call started',
  'executionPanel.event.tool_call_confirmation_required': 'Tool confirmation required',
  'executionPanel.event.tool_call_completed': 'Tool call completed',
  'executionPanel.event.done': 'Run completed',
  'executionPanel.event.cancelled': 'Run cancelled',
  'executionPanel.event.error': 'Run failed',

  'admin.title': 'Admin Console',
  'admin.description':
    'Manage users, review audit activity, and set workspace defaults.',
  'admin.backToChat': 'Back to chat',
  'admin.tabs.users': 'Users',
  'admin.tabs.audit': 'Audit Logs',
  'admin.tabs.policy': 'Workspace Policy',
  'admin.tabs.attachments': 'Attachments',
  'admin.loading': 'Loading admin data...',
  'admin.createUserTitle': 'Create user',
  'admin.createUserDescription':
    'New accounts inherit workspace defaults unless you set limits here.',
  'admin.createUser': 'Create user',
  'admin.creatingUser': 'Creating...',
  'admin.usersTitle': 'Users',
  'admin.lastLogin': 'Last login: {{value}}',
  'admin.never': 'Never',
  'admin.resetPassword': 'Reset password',
  'admin.resetPasswordTitle': 'Reset password for {{username}}',
  'admin.saveUser': 'Save',
  'admin.auditTitle': 'Audit logs',
  'admin.audit.filtersDescription':
    'Filter audit activity by actor, action, target type, or date range.',
  'admin.audit.time': 'Time',
  'admin.audit.actor': 'Actor',
  'admin.audit.action': 'Action',
  'admin.audit.target': 'Target',
  'admin.audit.summary': 'Summary',
  'admin.audit.system': 'System',
  'admin.audit.filterActor': 'Actor',
  'admin.audit.filterAction': 'Action',
  'admin.audit.filterTargetType': 'Target type',
  'admin.audit.filterDateFrom': 'Date from',
  'admin.audit.filterDateTo': 'Date to',
  'admin.audit.applyFilters': 'Apply filters',
  'admin.audit.resetFilters': 'Reset filters',
  'admin.audit.resultCount': 'Showing {{count}} of {{total}} logs',
  'admin.audit.empty': 'No audit logs match the current filters.',
  'admin.statsTitle': 'Usage overview',
  'admin.statsDescription':
    'Track workspace activity and the current provider distribution without leaving the admin console.',
  'admin.stats.totalUsers': 'Total users',
  'admin.stats.activeUsers': 'Active users',
  'admin.stats.recentUsers': 'Recent logins (7d)',
  'admin.stats.totalConversations': 'Total conversations',
  'admin.stats.totalMessages': 'Total messages',
  'admin.stats.totalAttachments': 'Total attachments',
  'admin.stats.activeProvidersSummary':
    '{{active}} active presets across {{configured}} configured providers',
  'admin.stats.activeProviderDistribution': 'Active provider distribution',
  'admin.stats.activeProviderDistributionHelp':
    'Shows which provider preset each user currently has active.',
  'admin.stats.noActiveProviders': 'No active provider presets yet.',
  'admin.stats.userCount': '{{count}} users',
  'admin.policyTitle': 'Workspace policy',
  'admin.policyDescription':
    'These defaults apply to newly created accounts. Leave limits blank for unlimited use.',
  'admin.policy.defaultRole': 'Default role',
  'admin.policy.defaultMaxConversations': 'Default max conversations',
  'admin.policy.defaultMaxAttachments': 'Default max attachments',
  'admin.policy.defaultDailyMessageLimit': 'Default daily message limit',
  'admin.attachments.title': 'Attachment retention',
  'admin.attachments.description':
    'Set how long unused attachments stay in storage and trigger manual cleanup when necessary.',
  'admin.attachments.retentionDays': 'Attachment retention (days)',
  'admin.attachments.totalCount': 'Currently storing {{count}} attachments.',
  'admin.attachments.savePolicy': 'Save attachment policy',
  'admin.attachments.userCleanupTitle': 'Delete by user',
  'admin.attachments.userCleanupDescription':
    'Remove every attachment uploaded by a specific user, including files still linked from historical messages.',
  'admin.attachments.deleteUser': 'Delete {{username}} attachments',
  'admin.attachments.dangerTitle': 'Workspace-wide cleanup',
  'admin.attachments.dangerDescription':
    'Use this only when you want to remove every stored attachment in the workspace.',
  'admin.attachments.deleteAll': 'Delete all attachments',
  'admin.attachments.confirmUserTitle': 'Delete {{username}} attachments',
  'admin.attachments.confirmUserDescription':
    'Delete all attachments uploaded by {{username}}. This cannot be undone.',
  'admin.attachments.confirmAllTitle': 'Delete all attachments',
  'admin.attachments.confirmAllDescription':
    'Delete every stored attachment for all users. This will break historical attachment links.',
  'admin.attachments.confirmDelete': 'Delete attachments',
  'admin.savePolicy': 'Save workspace policy',
  'admin.unlimited': 'Unlimited',
  'admin.field.role': 'Role',
  'admin.field.status': 'Status',
  'admin.field.maxConversations': 'Max conversations',
  'admin.field.maxAttachmentsPerMessage': 'Max attachments per message',
  'admin.field.dailyMessageLimit': 'Daily message limit',
  'admin.field.conversations': 'Conversations',
  'admin.field.attachments': 'Attachments',
  'admin.field.dailyMessages': 'Daily messages',
  'admin.feedback.userCreated': 'Created user {{username}}.',
  'admin.feedback.userUpdated': 'Updated {{username}}.',
  'admin.feedback.passwordReset': 'Password reset for {{username}}.',
  'admin.feedback.policyUpdated': 'Workspace policy updated.',
  'admin.feedback.attachmentsDeletedForUser':
    'Deleted {{count}} attachments for {{username}}.',
  'admin.feedback.attachmentsDeletedAll':
    'Deleted {{count}} attachments across the workspace.',
  'admin.role.admin': 'Admin',
  'admin.role.member': 'Member',
  'admin.status.active': 'Active',
  'admin.status.disabled': 'Disabled',

  'empty.title': 'How can I help you today?',
  'empty.withConversations':
    'Choose a conversation from the sidebar, or start a brand-new chat below.',
  'empty.withoutConversations':
    'Start typing below. Attachments, conversation settings, and markdown replies are all ready.',
  'empty.continueLastConversation': 'Continue last conversation',

  'message.editing': 'Editing',
  'message.edit': 'Edit',
  'message.failed': 'Failed',
  'message.stopped': 'Stopped',
  'message.thinking': 'Thinking...',
  'message.retry': 'Retry',
  'message.copy': 'Copy',
  'message.reasoning': 'Reasoning',
  'message.expandReasoning': 'Expand reasoning',
  'message.collapseReasoning': 'Collapse reasoning',
  'message.reasoningCollapsedHint': 'Click to expand and view the reasoning chain.',
  'message.copied': 'Copied',
  'message.code': 'code',

  'prompt.renameConversation': 'Rename conversation',
  'confirm.deleteConversation': 'Delete this conversation permanently?',
  'confirm.deleteProviderPreset':
    'Delete provider preset "{{name}}" permanently?',
  'confirm.deleteWorkflowPreset':
    'Delete workflow preset "{{name}}" permanently?',
  'confirm.deleteKnowledgeSpace':
    'Delete the knowledge space "{{name}}" and all indexed documents inside it?',
  'confirm.deleteKnowledgeDocument':
    'Delete the knowledge document "{{name}}" permanently?',

  'error.signIn': 'Unable to sign in right now',
  'error.backendUnavailable':
    'Unable to reach the backend service. Make sure the Go server is running.',
  'error.loadConversations': 'Unable to load conversations right now',
  'error.loadMessages': 'Unable to load message history right now',
  'error.loadChatSettings': 'Unable to load chat settings right now',
  'error.loadProviders': 'Unable to load AI provider settings right now',
  'error.saveProviderSettings': 'Unable to save AI provider settings',
  'error.activateProviderPreset': 'Unable to activate AI provider preset',
  'error.deleteProviderPreset': 'Unable to delete AI provider preset',
  'error.onlyAttachments':
    'Only image, PDF, Markdown, and TXT attachments are supported right now',
  'error.attachmentTooLarge': 'Attachment "{{name}}" is larger than 6 MB',
  'error.maxAttachments': 'You can attach up to {{count}} files',
  'error.deleteConversation': 'Unable to delete this conversation',
  'error.renameConversation': 'Unable to rename this conversation',
  'error.updateConversation': 'Unable to update this conversation',
  'error.archiveConversation': 'Unable to archive this conversation',
  'error.prepareMessage': 'Unable to prepare your message',
  'error.sendMessage': 'Unable to send your message',
  'error.completeReply': 'Unable to complete the reply. Please try again.',
  'error.updateMessage': 'Unable to update the message',
  'error.copyMessage': 'Unable to copy the message',
  'error.retryReply': 'Unable to retry the reply',
  'error.regenerateReply': 'Unable to regenerate the reply',
  'error.stopGeneration': 'Unable to stop generation',
  'error.saveSettings': 'Unable to save settings',
  'error.generationStopped': 'Generation stopped',
  'error.streamingFailed': 'Streaming failed',
  'error.assistantEndedUnexpectedly':
    'The assistant response ended unexpectedly.',
  'error.readSelectedAttachment': 'Unable to read the selected attachment',
  'error.readAttachmentNamed': 'Unable to read "{{name}}"',
  'error.uploadAttachment': 'Unable to upload attachment "{{name}}"',
  'error.uploadAttachmentGeneric': 'Unable to upload the selected attachment',
  'error.importConversation': 'Unable to import the selected Markdown file',
  'error.securityCurrentPasswordIncorrect': 'Current password is incorrect.',
  'error.securityPasswordRequired': 'Enter a password.',
  'error.securityPasswordTooShort':
    'Password must be at least 8 characters long.',
  'error.securityInvalidChangePasswordPayload':
    'The password change request is invalid.',
  'error.sessionExpired': 'Session expired. Please sign in again.',
  'error.loadAdminData': 'Unable to load admin data.',
  'error.createUser': 'Unable to create user.',
  'error.updateUser': 'Unable to update user.',
  'error.resetUserPassword': 'Unable to reset password.',
  'error.updateWorkspacePolicy': 'Unable to update workspace policy.',
  'error.loadAgentWorkspace': 'Unable to load the agent workspace.',
  'error.loadKnowledgeDocuments': 'Unable to load knowledge documents.',
  'error.saveRagProvider': 'Unable to save the RAG provider.',
  'error.activateRagProvider': 'Unable to activate the RAG provider.',
  'error.createKnowledgeSpace': 'Unable to create the knowledge space.',
  'error.updateKnowledgeSpace': 'Unable to update the knowledge space.',
  'error.deleteKnowledgeSpace': 'Unable to delete the knowledge space.',
  'error.createKnowledgeDocument': 'Unable to create the knowledge document.',
  'error.updateKnowledgeDocument': 'Unable to update the knowledge document.',
  'error.deleteKnowledgeDocument': 'Unable to delete the knowledge document.',
  'error.uploadKnowledgeDocuments': 'Unable to upload knowledge files.',
  'error.replaceKnowledgeDocumentFile': 'Unable to replace the knowledge file.',
  'error.createWorkflowPreset': 'Unable to create the workflow preset.',
  'error.updateWorkflowPreset': 'Unable to update the workflow preset.',
  'error.deleteWorkflowPreset': 'Unable to delete the workflow preset.',
  'error.updateWorkflowPresetSelection': 'Unable to clear the deleted workflow preset from this conversation.',

  'provider.loadingStatus': 'Loading provider status...',
  'provider.usingPreset':
    'Using preset "{{name}}" with model {{model}} for new requests.',
  'provider.usingFallbackModel': 'Using the server fallback with model {{model}}.',
  'provider.usingFallback': 'Using the server fallback.',
  'provider.notConfigured':
    'No AI provider is configured yet. Create a preset or complete the server fallback in data/app.json before sending messages.',

  'markdown.model': 'Model',
  'markdown.updated': 'Updated',
  'markdown.user': 'User',
  'markdown.assistant': 'Assistant',
} as const
