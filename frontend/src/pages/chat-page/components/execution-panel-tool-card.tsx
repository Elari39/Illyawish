...
export function ExecutionPanelToolCard({
  ...
  return (
    <section className="rounded-2xl border border-[var(--line)] bg-[var(--surface-strong)]/80 p-3">
      ...
      {tool.outputPreview ? (
        <p className="mt-3 rounded-2xl border border-[var(--line)] bg-[var(--app-bg)]/55 px-3 py-2.5 text-sm leading-6 text-[var(--muted-foreground)]">
          {tool.outputPreview}
        </p>
      ) : null}

      {tool.status === 'waiting_confirmation' ? (
        <div className="mt-3 rounded-2xl border border-[var(--status-waiting-border)] bg-[var(--status-waiting-bg)] p-3">
